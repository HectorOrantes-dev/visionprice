...
abstract class CollaborationRemoteDataSource {
  ...
  Future<InvitacionEntity> generarInvitacion(int proyectoId, String rol, List<String>? correos);
...
class CollaborationRemoteDataSourceImpl implements CollaborationRemoteDataSource {
  ...
  Future<MiembrosResultEntity> obtenerMiembros(int proyectoId) async {
    final data = await _client.getJson(ApiConfig.proyectoMiembros(proyectoId), auth: true);
    return MiembrosResultEntity.fromJson(data);
  }

  @override
  Future<void> quitarMiembro(int proyectoId, int usuarioId) async {
    await _client.deleteJson(ApiConfig.proyectoMiembro(proyectoId, usuarioId), auth: true);
  }

  @override
  Future<InvitacionEntity> generarInvitacion(int proyectoId, String rol, List<String>? correos) async {
  ...
  Future<List<InvitacionEntity>> obtenerInvitaciones(int proyectoId) async {
    final data = await _client.getJsonList(ApiConfig.proyectoInvitaciones(proyectoId), auth: true);
    return data.whereType<Map<String, dynamic>>().map(InvitacionEntity.fromJson).toList();
  }

  @override
  Future<void> revocarInvitacion(int proyectoId, int invitacionId) async {
    await _client.deleteJson(ApiConfig.proyectoInvitacionRevocar(proyectoId, invitacionId), auth: true);
  }

  @override
  Future<UnirseResultEntity> unirseAProyecto(String codigo) async {
    final body = <String, dynamic>{'codigo': codigo};
    final data = await _client.postJson(ApiConfig.proyectosUnirse, body, auth: true);
