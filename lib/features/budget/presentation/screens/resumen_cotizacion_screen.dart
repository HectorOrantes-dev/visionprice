...
class _ResumenCotizacionScreenState
  ...
  Widget build(BuildContext context) {
    ...
    final hayKit = List.generate(vm.superficies.length, (i) => i)
        .any((i) => vm.reglaDe(i).requiereKit);
    final haySimple = List.generate(vm.superficies.length, (i) => i)
        .any((i) => !vm.reglaDe(i).requiereKit);
                  ...
                  for (var i = 0; i < vm.superficies.length; i++) ...[
                    InkWell(
                      ...
                      onTap: () => Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (_) => ElegirMaterialScreen(
                              proyectoId: proyectoId, index: i),
                        ),
                      ),
                      child: Container(
              ...
              child: GradientButton(
                ...
                onPressed: vm.creando
                    ...
                    : () async {
                        ...
                        if (creadas.isEmpty) {
                          final estado =
                              ref.read(cotizacionWizardProvider(proyectoId));
                          if (estado.esPagoRequerido) {
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (_) => PlanPaywallScreen(
                                  code: estado.errorCode,
                                  message: estado.errorMessage ?? '',
                                ),
                              ),
                            );
                          }
